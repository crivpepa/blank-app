...
export default function PatientDetailScreen({ route, navigation }: Props) {
  ...
  const handleSaveNotas = async () => {
    setSavingNotas(true);
    try {
      await updatePatient(patient.id, { notas });
        ...
        <View key={session.id} style={styles.sessionCard}>
          <View style={styles.sessionHeader}>
            <View style={[styles.tipoBadge, { backgroundColor: (TIPO_COLORS[session.tipo] || PRIMARY) + '20' }]}>
              <Text style={[styles.tipoBadgeText, { color: TIPO_COLORS[session.tipo] || PRIMARY }]}>{session.tipo}</Text>
            </View>
          ...
          <View key={appt.id} style={styles.apptCard}>
            ...
            <View style={styles.apptInfo}>
              ...
              <View style={[styles.tipoBadge, { backgroundColor: (TIPO_COLORS[appt.type] || PRIMARY) + '20' }]}>
                <Text style={[styles.tipoBadgeText, { color: TIPO_COLORS[appt.type] || PRIMARY }]}>{appt.type}</Text>
              </View>
            </View>
  ...
  const renderNotas = () => (
    <View style={styles.notasCard}>
      ...
      />
      <TouchableOpacity style={styles.saveNotasBtn} onPress={handleSaveNotas} disabled={savingNotas}>
        {savingNotas
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={() => navigation.navigate('EditPatient', { patient })} style={styles.editBtn}>
...
const styles = StyleSheet.create({
  ...
  editBtn: { padding: 4 },
  scroll: { flex: 1 },
  heroCard: { backgroundColor: '#fff', alignItems: 'center', paddingVertical: 28, marginBottom: 12, elevation: 3, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.08, shadowRadius: 4 },
  avatarCircle: { width: 80, height: 80, borderRadius: 40, backgroundColor: PRIMARY, justifyContent: 'center', alignItems: 'center', marginBottom: 12 },
  ...
  statsRow: { flexDirection: 'row', backgroundColor: '#fff', marginBottom: 12, elevation: 3, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.08, shadowRadius: 4 },
  ...
  sessionCard: { backgroundColor: '#fff', borderRadius: 12, padding: 14, marginBottom: 10, elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 4 },
  ...
  apptCard: { backgroundColor: '#fff', borderRadius: 12, padding: 14, marginBottom: 10, flexDirection: 'row', alignItems: 'center', elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 4 },
