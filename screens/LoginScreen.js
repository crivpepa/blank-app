...
const DEMO_EMAIL = 'admin@recovery.com';
  ...
  const handleLogin = async () => {
    ...
    setTimeout(() => {
      if (email.trim().toLowerCase() === DEMO_EMAIL && password === DEMO_PASSWORD) {
        login({ email: email.trim().toLowerCase(), name: 'Administrador', role: 'Admin' });
      } else {
        Alert.alert('Credenciales incorrectas', 'El correo o contraseña no son válidos. Use las credenciales de demostración.');
      }
      setLoading(false);
    }, 800);
  };
