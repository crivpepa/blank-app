...
export default function CitasScreen({ navigation }: Props) {
  const days = getNext7Days();
  const [selectedDate, setSelectedDate] = useState(formatDate(new Date()));
  const { appointments, loading, load, error } = useAppointments();
  ...
  // Sync local state when hook loads
  React.useEffect(() => {
    setLocalAppointments(appointments);
  }, [appointments]);

  const todayAppts = localAppointments.filter(a => a.date === selectedDate);
  ...
  const handleLongPress = (appt: Appointment) => {
    ...
    Alert.alert(
      ...
      [
        ...
        { text: 'Marcar como completada', onPress: () => markAs(appt.id, 'Completada') },
        { text: 'Cancelar cita', style: 'destructive', onPress: () => markAs(appt.id, 'Cancelada') },
      ]
    );
  };

  const markAs = async (id: string, status: Appointment['status']) => {
    try {
  ...
  const renderGroup = (title: string, apptList: Appointment[]) => {
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.daySelector}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.daySelectorContent}>
          {days.map(day => {
            ...
            const count = localAppointments.filter(a => a.date === dateStr).length;
            return (
              <TouchableOpacity
                key={dateStr}
              ...
              >
                ...
                {count > 0 && <View style={[styles.dayDot, isSelected && styles.dayDotActive]} />}
              </TouchableOpacity>
            );
          })}
        </ScrollView>
        ...
        <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
          {todayAppts.length === 0 ? (
            <View style={styles.emptyState}>
              ...
              <TouchableOpacity style={styles.emptyBtn} onPress={() => navigation.navigate('AgendarCita')}>
      ...
      <TouchableOpacity style={styles.fab} onPress={() => navigation.navigate('AgendarCita')} activeOpacity={0.85}>
...
const styles = StyleSheet.create({
  ...
  header: { backgroundColor: PRIMARY, paddingHorizontal: 20, paddingTop: 10, paddingBottom: 16 },
  ...
  dayBtn: { width: 52, height: 76, borderRadius: 18, backgroundColor: 'rgba(255,255,255,0.15)', alignItems: 'center', justifyContent: 'center' },
  ...
  groupHeader: { flexDirection: 'row', alignItems: 'center', paddingVertical: 8, marginBottom: 4 },
