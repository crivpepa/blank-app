...
export const getPatient = async (id: string): Promise<Patient | null> => {
  if (isFirebaseConfigured()) {
    try {
      ...
      if (docSnap.exists()) return { id: docSnap.id, ...docSnap.data() } as Patient;
      return null;
    } catch (error: any) {
...
export const createPatient = async (data: Omit<Patient, 'id' | 'createdAt'>): Promise<Patient> => {
  if (isFirebaseConfigured()) {
    try {
  ...
  const newPatient: Patient = { id: `p${Date.now()}`, ...data, createdAt: new Date() };
...
export const deletePatient = async (id: string): Promise<boolean> => {
  ...
  const idx = DEMO_DATA.findIndex(p => p.id === id);
  if (idx !== -1) DEMO_DATA.splice(idx, 1);
  return true;
};
